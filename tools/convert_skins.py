...
"""Convert the skin images in assets to the flash data headers the game includes.
  ...
  assets/skins  -> source/blockdude_embedded/images   (16x16 tiles)
  assets/skins2 -> source/blockdude_embedded/images2  (8x8 tiles)

Full screen images (background, intro1-3, titlescreen) become run length encoded
headers (<name>_RLE565.h, see png2rle565.py), the tile images (*-table-*.png) become
raw RGB565 little endian headers (<name>_RGB565_LE.h, see png2rgb565.py).
...
SOURCE_DIR = os.path.join(ROOT, "source", "blockdude_embedded")
...
def convert_image(skins_dir, skin, png, out_dir):
    ...
    prefix = png2rle565.SKIN_PREFIX[skin]
...
def convert_set(skins_dir, out_dir):
    ...
    made = {}
    skipped = []
    for skin in png2rle565.SKIN_PREFIX:
