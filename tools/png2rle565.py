...
The raw RGB565 full screen images (5 per skin, 32KB each) do not all fit in the
ESP8266's ~1MB of flash that can be mapped for code and PROGMEM, so they are
stored compressed and drawn with pushImageRLE() in gamefuncs.cpp.
...
defaults to ../assets/skins and ../source/blockdude_embedded/images next to this script
"""
import os
import sys
from PIL import Image

FULLSCREEN_IMAGES = ["background", "intro1", "intro2", "intro3", "titlescreen"]
...
def main():
    ...
    images_dir = sys.argv[2] if len(sys.argv) > 2 else os.path.join(here, "..", "source", "blockdude_embedded", "images")
